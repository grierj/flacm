# FLA Configuration Management (FLACM) Design
...
1. Check for .reboot file, if present, download the new version and exec to it.
1. Determine roles and domains
1. Download OS, Domain, Roles, and Host configs and scripts from FLACM repository.
1. Run OS pre
1. Run OS fix
1. scrub false root, remove any {file}.part if a {file}.whole exists
1. copy whole files into place, append part files to existing configs
1. Run OS post
1. repeat previous 5 steps for Domain, Roles, Host
1. During the Roles portion:
   * If there are any removed roles, run uninstall
   * If there are any new roles, run the pre, then fix, then install
1. Check to see if Phase 3 is in cron, schedule if not
1. Check to see if there is a /etc/flacm/.reboot file. Reboot if so.
1. Check to see if the reboot time has been reached, Reboot if so.
